...
class TagsQuotesSource(private val apiService: ApiService, private val tag: String) :
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, QuoteResult> {
        return try {
            ...
            val tagData = apiService.getTagsQuotes(tags = tag, page = nextPage)
            LoadResult.Page(
                data = tagData.results,
                nextKey = if (tagData.results.isEmpty()) null else nextPage + 1,
